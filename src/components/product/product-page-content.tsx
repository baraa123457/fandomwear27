...
/**
 * The shared body of a product detail page. Pulled out into its own client
 * component so it can be reused both by the statically-generated route
 * (products known at build time) and by a client-side lookup for products
 * created live in the admin catalog manager, which only exist in this
 * browser's shared CatalogContext / localStorage — there is no backend, so
 * a build-time page can never know about them.
 */
export function ProductPageContent({ product }: { product: Product }) {
  ...
  // Reviews now live in Supabase (see PHASE 4). The procedural generator is
  // kept only as an offline/error fallback, and as the seed source for
  // supabase/seed.sql, so the tab renders identically either way.
  ...
  useEffect(() => {
    let cancelled = false;
    setReviews(getReviewsForProduct(product.id, product.rating));
    (async () => {
      try {
        ...
        console.warn("[reviews] Falling back to generated reviews — Supabase fetch failed:", err);
      }
    })();
    return () => {
      cancelled = true;
      ...
      <div className="mt-16">
        <Tabs defaultValue="description">
          ...
          <TabsContent value="reviews">
            <Reviews reviews={reviews} averageRating={product.rating} />
          </TabsContent>
        </Tabs>
      </div>
